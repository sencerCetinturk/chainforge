...
class ChangeLogger {
    ...
    async log(change, agentKey, model, intent) {
        if (!this.logDir)
            return null;
        const diff = this.makeUnifiedDiff(change.filePath, change.originalContent || "", change.newContent);
    ...
    async getRecentForFiles(filePaths, maxPerFile = 2) {
        if (!this.logDir || filePaths.length === 0)
            return [];
        ...
        try {
            ...
            for (const name of names) {
                if (all.length >= filePaths.length * maxPerFile)
                    break;
                const uri = vscode.Uri.joinPath(this.logDir, name);
                try {
                    const e = JSON.parse(Buffer.from(await vscode.workspace.fs.readFile(uri)).toString("utf8"));
                    if (!wanted.has(e.filePath))
                        continue;
                    perFileCount[e.filePath] = (perFileCount[e.filePath] || 0);
                    if (perFileCount[e.filePath] >= maxPerFile)
                        continue;
    ...
    async readSince(sinceTimestamp) {
        if (!this.logDir)
            return [];
        const entries = [];
    ...
    async uniqueFileName(targetPath) {
    ...
    makeUnifiedDiff(path, oldStr, newStr) {
    ...
    countLines(oldStr, newStr) {
        const oldSet = oldStr.split("\n");
        const newSet = newStr.split("\n");
        let start = 0;
        while (start < oldSet.length && start < newSet.length && oldSet[start] === newSet[start])
            start++;
        let endOld = oldSet.length - 1, endNew = newSet.length - 1;
        while (endOld >= start && endNew >= start && oldSet[endOld] === newSet[endNew]) {
            endOld--;
            endNew--;
        }
...
//# sourceMappingURL=changeLogger.js.map