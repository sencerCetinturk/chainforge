...
class Inspector {
    ...
    async inspect(forceFull = false) {
        ...
        if (!lastCheck) {
            ...
            for (const f of codeFiles) {
                ...
                if (content !== null)
                    filesToCheck.push({ path: f.path, content });
            }
        }
        else {
    ...
    async inspectFile(path, content) {
...
SADECE JSON formatında yanıtla (başka hiçbir şey yazma):
{
  ...
  "issues": [
    ...
    async writeInspectionReport(results, summary) {
        if (!this.logDir)
            return null;
        try {
        ...
        try {
            ...
            for (const [name, type] of files) {
                if (type !== vscode.FileType.File || !name.endsWith(".json"))
                    continue;
                if (sinceKey && name.split("__")[0] <= sinceKey)
                    continue;
...
Inspector.LAST_CHECK_KEY = "chainforge.lastInspection";
