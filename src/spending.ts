...
// ── Dinamik fiyat önbelleği (OpenRouter /api/v1/models → ücretsiz, token harcamaz) ──

interface PricingCache {
  updatedAt: number;
  models: Record<string, { inputPrice: number; outputPrice: number }>; // $/M token
}

let dynamicPricing: PricingCache | null = null;
 ...
 */
export async function refreshPricingCache(context: vscode.ExtensionContext): Promise<void> {
  // Önce globalState'ten yükle
  const cached = context.globalState.get<PricingCache>("chainforge.pricingCache");
  ...
  try {
    ...
    const models: Record<string, { inputPrice: number; outputPrice: number }> = {};
    if (response?.data && Array.isArray(response.data)) {
      for (const m of response.data) {
        const id = (m.id || "").toLowerCase();
        // OpenRouter API fiyatı token başına döner → $/M token'a çevir
        const promptPrice = parseFloat(String(m.pricing?.prompt || "0")) * 1_000_000;
        const completionPrice = parseFloat(String(m.pricing?.completion || "0")) * 1_000_000;
...
const STATIC_PRICING: [string, number, number][] = [
  // ── Anthropic Claude ──
...
export function estimateCost(model: string, promptTokens: number, completionTokens: number): number {
  ...
  if (dynamicPricing) {
    ...
    for (const c of candidates) {
      ...
      if (dyn) {
        return (promptTokens / 1_000_000 * dyn.inputPrice) + (completionTokens / 1_000_000 * dyn.outputPrice);
      }
    }
  }
