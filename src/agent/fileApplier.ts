...
import { validateRelativeFilePath } from "../validator";
...
export class FileApplier {
  ...
  async applyWithApproval(change: FileChange, autoApprove = false): Promise<boolean> {
    ...
    const pathCheck = validateRelativeFilePath(change.filePath);
  ...
  async applyBatch(changes: FileChange[]): Promise<{ applied: FileChange[]; skipped: FileChange[] }> {
    ...
    if (dirty) {
      const autoCheckpoint = vscode.workspace.getConfiguration("chainforge").get<boolean>("autoCheckpoint", false);
      if (autoCheckpoint) {
        ...
        if (committed) {
          ...
          const proceed = await vscode.window.showWarningMessage(
            "Workspace'te commit edilmemiş git değişiklikleri var ve otomatik checkpoint başarısız oldu. ChainForge bunların üzerine yazabilir.\nYine de devam edilsin mi?",
            { modal: true },
            "Devam Et"
          );
          if (proceed !== "Devam Et") return { applied, skipped: changes };
        }
      } else {
        const proceed = await vscode.window.showWarningMessage(
          "Workspace'te commit edilmemiş git değişiklikleri var. ChainForge bunların üzerine yazabilir.\nYine de devam edilsin mi?",
          { modal: true },
          "Devam Et"
        );
        if (proceed !== "Devam Et") return { applied, skipped: changes };
    ...
    const choice = await vscode.window.showInformationMessage(
      `ChainForge ${changes.length} dosyada değişiklik önerdi.`,
      { modal: true },
      "Her birini incele", "Tümünü uygula", "İptal"
    );
    ...
    for (const change of changes) {
      ...
      if (ok) applied.push(change); else skipped.push(change);
    }
  ...
  private async isGitWorkspaceDirty(): Promise<boolean> {
    const folders = vscode.workspace.workspaceFolders;
    if (!folders) return false;
    const cwd = folders[0].uri.fsPath;
    return new Promise(resolve => {
      exec("git status --porcelain", { cwd, timeout: 5000 }, (err, stdout) => {
        if (err) return resolve(false); // git yok veya repo değil → engelleme
  ...
  private async createCheckpointCommit(): Promise<boolean> {
    const folders = vscode.workspace.workspaceFolders;
    if (!folders) return false;
    const cwd = folders[0].uri.fsPath;
    const run = (cmd: string): Promise<boolean> => new Promise(resolve => {
      exec(cmd, { cwd, timeout: 10000 }, (err) => resolve(!err));
    });
  ...
  async revertBatch(changes: FileChange[]): Promise<{ reverted: number; failed: string[] }> {
    const folders = vscode.workspace.workspaceFolders;
    if (!folders) return { reverted: 0, failed: changes.map(c => c.filePath) };
    const root = folders[0].uri.fsPath;
    let reverted = 0;
    const failed: string[] = [];
  ...
  private async showApprovalDialog(change: FileChange, uri: vscode.Uri): Promise<boolean> {
  ...
  private async openDiffPreview(change: FileChange, uri: vscode.Uri): Promise<void> {
    try {
      ...
      await vscode.commands.executeCommand(
        "vscode.diff", leftDoc.uri, rightDoc.uri,
        `${change.filePath} (Mevcut ↔ Önerilen)`,
        { preview: true }
      );
