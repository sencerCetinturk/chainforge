...
import { CodeIndexStore, CodeHeader } from "./codeIndex";
import { validateRelativeFilePath } from "../validator";
import { FlowStepEvent } from "../telemetry";
import {
...
export class Orchestrator {
  // Postacı önce config'deki TÜM "routing" rollü agent'ları tanımlama sırasıyla dener,
  // hepsi başarısız olursa ücretsiz model zincirine düşer (provider hatalarına karşı dayanıklı).
  private routingAgentKeys: string[];
  private freeModels: string[];
  private flowEvents: FlowStepEvent[] = [];
  ...
  constructor(
    ...
    private codeIndexStore?: CodeIndexStore  // Postacı'nın dosya/satır konumlarını "rahat bulması" için
  ) {
  ...
  async run(userPrompt: string, codingTaskType: string): Promise<OrchestratorResult> {
    ...
    const flowEvents: FlowStepEvent[] = [];
    const recordFlow = (e: FlowStepEvent) => flowEvents.push(e);
  ...
  private async makePlan(userPrompt: string, relevant: ScannedFile[], all: ScannedFile[], codeHeaders: CodeHeader[] = []): Promise<TaskPlan> {
    const headersByFile = new Map<string, CodeHeader[]>();
    for (const h of codeHeaders) {
      if (!headersByFile.has(h.file)) headersByFile.set(h.file, []);
...
- Sadece hesaplama/formül gerekiyorsa needsMath=true.`;
  ...
  private async extractExcerpts(
    userPrompt: string,
    plan: TaskPlan,
    files: { path: string; content: string }[],
    codeHeaders: CodeHeader[] = []
...
- Yeni satır eklenecekse (ör. dosya sonuna) uygun bir çapa (ör. son birkaç satır) alıntıla.`;
    ...
    const res = await this.postmanCall(
      "Sen bir kod konumlandırma uzmanısın — kod YAZMAZSIN, sadece ilgili bölümü olduğu gibi (kopyala-yapıştır) bulursun. Sadece geçerli JSON döndürürsün, asla açıklama yazmazsın.",
      prompt
    );
    if (!res.success) return result;

    const parsed = this.extractJson(res.content);
    if (!parsed || !Array.isArray(parsed.files)) return result;

    for (const entry of parsed.files) {
      const path = entry?.path;
      const excerpt = entry?.excerpt;
      if (typeof path !== "string" || typeof excerpt !== "string") continue;
      if (excerpt.trim().length < 20) continue; // çok kısa alıntı güvenilmez — tam dosyaya düş
...
<<<END>>>`;
  ...
  private addChange(
    ...
    const pathCheck = validateRelativeFilePath(filePath);
    if (!pathCheck.valid) {
